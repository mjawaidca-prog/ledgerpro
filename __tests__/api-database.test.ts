...
import { withIdempotency, IdempotencyContext } from '@/lib/api/idempotency';
...
suite('API accounting transaction integration (PostgreSQL)', () => {
  ...
  const context = (requestKey: string): IdempotencyContext => ({ requestKey, apiKeyId, companyId, method: 'POST', path: '/api/v1/payments' });
