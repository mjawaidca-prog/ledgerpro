...
async function main() {
  ...
  const plans = await Promise.all([
    ...
    db.plan.upsert({ where: { id: 'plan_pro' }, update: {}, create: { id: 'plan_pro', name: 'Pro', stripePriceId: 'price_pro_monthly', monthlyPrice: 79, annualPrice: 790, maxUsers: 10, maxCompanies: 5, maxTransactions: 10000, maxBankAccounts: 10, csvExport: true, pdfReports: true, bankFeeds: true, customReports: true, prioritySupport: true, whiteLabel: false, apiAccess: true } }),
    db.plan.upsert({ where: { id: 'plan_enterprise' }, update: {}, create: { id: 'plan_enterprise', name: 'Enterprise', stripePriceId: 'price_enterprise_monthly', monthlyPrice: 199, annualPrice: 1990, maxUsers: 50, maxCompanies: 25, maxTransactions: 100000, maxBankAccounts: 50, csvExport: true, pdfReports: true, bankFeeds: true, customReports: true, prioritySupport: true, whiteLabel: true, apiAccess: true } }),
  ]);
