...
// POST /api/v1/invoices/[id]/void — void a draft (deleted) or a posted
// invoice (tax-posting reversal, only when no payments remain). write_posting.
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    console.error('POST /api/v1/invoices/[id]/void error:', err);
    return NextResponse.json({ error: { code: 'internal_error', message: 'Failed to void invoice.' } }, { status: 500 });
  }

  await auditLog(context!.companyId, undefined, 'api.invoice.void', 'invoice', params.id, undefined, {
    apiKeyId: context!.apiKeyId,
