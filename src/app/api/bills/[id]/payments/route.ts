...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (guardError) return guardError;
    const result = await reverseDocumentPayment({ documentId: params.id, paymentEntryId, companyId, documentType: 'bill', reversalDate, userId: userId ?? undefined });
    await auditLog(companyId, userId, 'bill.payment.reverse', 'bill', params.id, { paymentEntryId, reversalEntryId: result.reversal.id, reversalDate } as any);
