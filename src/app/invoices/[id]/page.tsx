...
export default function EditInvoicePage() {
  ...
  // Fetch categories. Existing documents retain their saved historical rate.
  useEffect(() => {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setIssueDate(format(new Date(inv.issueDate), 'yyyy-MM-dd'));
        setDueDate(format(new Date(inv.dueDate), 'yyyy-MM-dd'));
  ...
  async function handleSave(newStatus: string) {
    ...
    const payload = {
      ...
      issueDate, dueDate, terms, subtotal, taxRate, taxAmount, total,
      status: newStatus,
    ...
    try {
      ...
      if (newStatus === 'sent') setStatus('sent');
    } catch (err) {
  ...
  async function handleVoid() {
    ...
    try {
      ...
      setToast({ message: 'Failed to void invoice.', type: 'danger' });
    } finally {
      setSaving(false);
    }
  }
  ...
  const statusVariant =
    status === 'paid' ? 'paid' as const : status === 'overdue' ? 'overdue' as const :
    ...
    <AppShell>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex-1">
          <h1 className="t-h1">Invoice {id}</h1>
        ...
        {!isLocked && (
          <>
            ...
            <Button onClick={() => handleSave('sent')} disabled={saving}>
              ...
              {status === 'draft' ? 'Send Invoice' : 'Update'}
            </Button>
          </>
        )}
      ...
      <div className={cn('grid grid-cols-[1fr_340px] gap-6', isLocked && 'pointer-events-none opacity-60')}>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-[var(--shadow-sm)] p-5">
            <label className="font-mono text-micro uppercase tracking-[0.08em] text-[var(--text-muted)] block mb-2">Bill To</label>
            {selectedCustomer && (
          ...
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-[var(--shadow-sm)] p-5">
            ...
            <textarea
              rows={3} placeholder="Payment instructions or internal notes..."
        ...
        <div className="space-y-4">
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-[var(--shadow-sm)] p-5 space-y-4">
            <div className="field">
              ...
              <input type="date" value={issueDate} onChange={(e) => setIssueDate(e.target.value)} className="input" readOnly={isLocked} />
            </div>
            <div className="field">
                ...
                {['Net 15', 'Net 30', 'Net 60'].map((t) => (
                  <button key={t} type="button" disabled={isLocked}
                    ...
                    className={cn(
                      ...
                      terms === t ? 'border-[var(--border-focus)] bg-[var(--primary-soft)] text-[var(--primary)]' : 'border-[var(--border)] text-[var(--text-muted)] hover:text-[var(--text-strong)]'
                    )}
                  >{t}</button>
                ))}
              </div>
          ...
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-[var(--shadow-sm)] overflow-hidden">
            ...
            <div className="px-5 py-4 space-y-3">
              ...
              {status === 'paid' && (
                <div className="flex justify-between pt-2 border-t border-[var(--success-soft-border)]">
                  <span className="text-sm font-medium text-[var(--success)]">Paid</span>
                  <span className="font-mono tabular-nums text-sm font-semibold text-[var(--success)]">
                    {money(paidAmount)}
                  </span>
                </div>
              )}
